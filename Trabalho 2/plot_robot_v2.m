...
function plot_robot(X, Y, X1, Y1, T1, X2, Y2, T2, Time, Radius)

    % Dimensões do robô
    Solido = [-1, 1, 2, 1, -1, -1.5, -1.5; -1, -1, 0, 1, 1, 1, -1];
    Roda_L = [-1 0 0 -1 -1; 1 1 1.5 1.5 1];
    Roda_R = [-1 0 0 -1 -1; -1 -1 -1.5 -1.5 -1];
    FrameX = [0 3; 0 0];
    FrameY = [0 0; 0 3];
    ...
    plot(X, Y, 'b', X1, Y1, '-y', X2, Y2, '-g', 'linewidth', 3)
    grid on; hold on; axis equal; axis([-Radius-10 Radius+10 -10 2*Radius+10]);
    ...
    title(['Thi: ', num2str(rad2deg(T1(end)), '%10.3f'), '° | ' , ...
           ...
           'Tempo: ', num2str(Time(end), '%10.3f'), 's']);
